...
void send_dns(int s,  char  buf[] ,struct sockaddr_in dest){
	...
	char   root[24],send_buf[65532];root[23]='\0';
	int stop=0;
	...
	dns->qr=1;	dns->aa=1;	dns->ra=1;dns->ad=0;	dns->rcode=0;
	dns->ans_count =0;
	...
	// unsigned char  test[]="example.org";
	// printf("test %s \n",test);
...
void ngethostbyname(unsigned char *host, int query_type,int cs,struct sockaddr_in csin) {
